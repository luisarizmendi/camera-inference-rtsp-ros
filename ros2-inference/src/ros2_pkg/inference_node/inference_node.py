...
class InferenceNode(Node):
    ...
    def __init__(self):
        ...
        # ── Load YOLO model ───────────────────────────────────────────────────
        self.get_logger().info("Loading YOLO model (first run downloads weights) ...")
        from ultralytics import YOLO

        # Look for the model in YOLO_MODELS_DIR first (pre-downloaded at build
        # time), then fall back to letting Ultralytics download it to /tmp.
    ...
    def _open_capture(self) -> Optional[cv2.VideoCapture]:
        ...
        # Minimise OpenCV buffer to reduce latency
    ...
    def _inference_loop(self):
        ...
        while not self._stop.is_set():
            ...
            while not self._stop.is_set():
                ...
                # Run YOLO — source frame dimensions for coordinate scaling
    ...
    def _run_inference(self, frame: np.ndarray, orig_w: int, orig_h: int):
        """Run YOLO on frame and publish detections scaled to original dimensions."""
        try:
            results = self._model(
                frame,
        ...
        # Scale factors from inference size back to original frame size
        ...
        if self.verbose and msg.detections:
            self.get_logger().info(
                f"Published {len(msg.detections)} detection(s)"
            )
